...
const customErrorMap: ZodErrorMap = (issue, _ctx) => {
  ...
  switch (issue.code) {
    ...
    case ZodIssueCode.invalid_literal:
      message = `無効なリテラル値、期待値：${JSON.stringify(issue.expected, util.jsonStringifyReplacer)}`;
      break;
    case ZodIssueCode.unrecognized_keys:
      message = `オブジェクト内で認識されないキー：${util.joinValues(issue.keys, ', ')}`;
      break;
    case ZodIssueCode.invalid_union:
      message = `無効な入力`;
      break;
    case ZodIssueCode.invalid_union_discriminator:
      message = `無効なディスクリミネータ値。期待値：${util.joinValues(issue.options)}`;
      break;
    case ZodIssueCode.invalid_enum_value:
      message = `無効な列挙値。期待値：${util.joinValues(issue.options)}、受け取った値：'${issue.received}'`;
      break;
    ...
    case ZodIssueCode.too_small:
      if (issue.type === 'array')
        ...
        message = `日付は${
          issue.exact ? `ちょうど` : issue.inclusive ? `以上` : `より大きい`
        }${String(new Date(Number(issue.minimum)))}である必要があります`;
      else message = '無効な入力';
      break;
    case ZodIssueCode.too_big:
      if (issue.type === 'array')
        ...
        message = `日付は${
          issue.exact ? `ちょうど` : issue.inclusive ? `以下` : `未満`
        }${String(new Date(Number(issue.maximum)))}である必要があります`;
      else message = '無効な入力';
      break;
